<template >
  <div id="login-box">
    <div class="left">
      ...
      <router-link to="/registre">Sign In</router-link>
      <p v-if="error" class="mt-3" style="color: red;">{{ error }}</p>
    </div>
      
  <div class="right">
    <span class="loginwith">Login  with <br>  Notomatic</span>
  ...
  export default {
    ...
    methods: {
       ...
       async login(){
        ...
        const User = await axios.post('http://localhost:5000/api/connexion',{...Data})
        ...
        this.error = 'Invalid . Please try again.';
      }
    }
  };
  </script>
